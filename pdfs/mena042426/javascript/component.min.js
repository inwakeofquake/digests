...
m){function n(t){return function(r){p[t]=r;q--;0==q&&l(p)}}var p=[],q=0;do p.push(void 0),q++,c(h.value).callWhenSettled_(n(p.length-1),m),h=k.next();while(!h.done)})};return e},"es6","es3");$jscomp.findInternal=function(a,b,c){a instanceof String&&(a=String(a));for(var d=a.length,e=0;e<d;e++){var g=a[e];if(b.call(c,g,e,a))return{i:e,v:g}}return{i:-1,v:void 0}};$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(b,c){return $jscomp.findInternal(this,b,c).v}},"es6","es3");
...
function(l,m,n,p){var q=this,t,r,x,u,v;return $jscomp.asyncExecutePromiseGeneratorProgram(function(w){switch(w.nextAddress){case 1:return t=$("<div style='position:relative;width:100%;height:100%;'></div>"),r=$(n),l&&-1<l.indexOf("none")&&!l.startsWith("data:image")&&m&&0>m.indexOf("none")&&(r=$(m)),void 0===p&&(p=n),w.setCatchFinallyBlocks(2),w.yield(q.createCompositeImage(r,l),4);case 4:(x=w.yieldResult)&&(r=$(x));w.leaveTryBlock(3);break;case 2:u=w.enterCatchBlock(),console.error("Canvas\u935a\u581f\u579a\u6fb6\u8fab\u89e6:",
...
q.printArea.append(q.printArray[v]);q.printArea.printArea({onReady:q.hidePrintLoading.bind(q)})}w.jumpToEnd()}})};e=BookInfo.getBook().pageWidth;g=BookInfo.getBook().pageHeight;f={};for(k=0;k<a.length;f={$jscomp$loop$prop$page$6:f.$jscomp$loop$prop$page$6},k++)f.$jscomp$loop$prop$page$6=a[k],getPageDir(f.$jscomp$loop$prop$page$6,"normal").then(function(l){return function(m){var n=e,p=g;if(bookConfig.normalSuffix&&"zip"===bookConfig.normalSuffix||-1<m.indexOf("zip"))window.printLoadStack||b.initPrintStack(a),
n=2*e,p=2*g;"object"!=typeof m&&m.match(/none/)?b.getPrintImgType(l.$jscomp$loop$prop$page$6).then(function(q){pageResourceLoad(l.$jscomp$loop$prop$page$6,q,d.bind(b),n,p)}):pageResourceLoad(l.$jscomp$loop$prop$page$6,"normal",d.bind(b),n,p)}}(f));h.jumpToEnd()})},createCompositeImage:function(a,b){var c=this;return new Promise(function(d,e){try{var g=document.createElement("canvas"),f=g.getContext("2d"),k=new Image;k.crossOrigin="anonymous";k.onload=function(){var h,l,m,n;return $jscomp.asyncExecutePromiseGeneratorProgram(function(p){if(1==
p.nextAddress)return p.setCatchFinallyBlocks(2),g.width=k.naturalWidth,g.height=k.naturalHeight,f.drawImage(k,0,0),p.yield(c.createWatermarkWithPageWatermark(k.naturalWidth,k.naturalHeight),4);if(2!=p.nextAddress)return(h=p.yieldResult)?f.drawImage(h,0,0):console.log("watermarkCanvas error"),l=g.toDataURL("image/png"),m=document.createElement("img"),m.src=l,m.style.cssText=a[0].style.cssText,d(m),p.leaveTryBlock(0);n=p.enterCatchBlock();console.error("canvas error:",n);d(null);p.jumpToEnd()})};k.onerror=
function(){console.log("img.onerror:",b);d(null)};k.src=b}catch(h){console.log("createCompositeImage error:",h),d(null)}})},createWatermarkWithPageWatermark:function(a,b){return new Promise(function(c){if(parseBool(bookConfig.PrintButtonVisible)&&bookConfig.printWatermark)try{var d=document.createElement("canvas");d.width=a;d.height=b;var e=new WatermarkCore(d),g=BookInfo.getBook().pageWidth,f=BookInfo.getBook().pageHeight,k=Math.min(a/g,b/f),h={position:bookConfig.watermarkPosition,width:(bookConfig.watermarkWidth||
100)*k,height:(bookConfig.watermarkHeight||50)*k,opacity:bookConfig.watermarkOpacity,spacing:(bookConfig.watermarkSpacing||50)*k,rotation:bookConfig.watermarkRotation,watermarkEdgeMargin:(bookConfig.watermarkEdgeMargin||12)*k,watermarkImage:new Image};if(bookConfig.printWatermarkFile){if(h.watermarkImage.src=bookConfig.printWatermarkFile,h.watermarkImage.onload=function(){try{bookConfig.watermarkHeight||(h.height=h.width*h.watermarkImage.height/h.watermarkImage.width),e.applyWatermark(h),c(d)}catch(l){console.error("\u59d8\u6751\u5d43\u6434\u65c2\u6564\u6fb6\u8fab\u89e6:",
...
this;this.downloadAll.click(function(c){this.buttonEvent(a,c.delegateTarget)}.bind(this));this.downloadCurrent.click(function(c){this.buttonEvent(a,c.delegateTarget)}.bind(this));this.downloadAttachments.click(function(c){this.buttonEvent(a,c.delegateTarget)}.bind(this));this.verify=!1;this.downloadButton.click(function(){var c;return $jscomp.asyncExecutePromiseGeneratorProgram(function(d){if(b.restricted)return d.return();c=function(){var e,g,f,k;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){e=
b.downloadButton.downloadType;g=b.pageIndex;f=b.currentAttachmentName||"";k=b.selectAttachments[0].value;return h.yield(b.service.doDownload(e,g,f,k,b.downloadButton,b.downloadingTips),0)})};if(b.service.isRestricted())return d.return();b.service.ensurePasswordThen(c);d.jumpToEnd()})})},buttonEvent:function(a,b){var c=a.stage.find(".on");c.find("img").attr("src",this.downloadOffUrl);c.find("img")[0].colorChanged=!1;c.removeClass("on");$(b).find("img").attr("src",this.downloadOnUrl);$(b).find("img")[0].colorChanged=
...
DownloadService.prototype.doDownload=function(a,b,c,d,e,g){var f=this,k,h,l,m,n,p,q,t,r,x;return $jscomp.asyncExecutePromiseGeneratorProgram(function(u){if(1==u.nextAddress)return k={downloadAll:"download_full_publication",downloadCurrent:"download_page_"+b,downloadAttachments:"download_attachment_"+c},"downloadCurrent"===a?BookEvent.trigger("customEvent",[k[a],{downloaded_page:b}]):BookEvent.trigger("customEvent",[k[a]]),u.yield(f.buildUrl(a,b,d),2);h=u.yieldResult;if("object"===typeof h){if("downloadCurrent"===
a)f._loadImageToCanvas(h,function(v,w){f._createAndTriggerDownload(w.toDataURL("image/png"),"page"+b+".png")});else for(l=0;l<h.length;l++)m=h[l],bookConfig.isEncryptionBook||(m=m.removeStartFrom("?")),window.open(m,"_blank");return u.return()}if("string"===typeof h&&-1<h.indexOf(".zip")&&"downloadAttachments"!==a)return f._pdfLoadingTask&&(f._pdfLoadingTask.pause(),f._pdfLoadingTask.destroy()),f._pdfLoadingTask=new PdfLoadingTask(b,"normal",windowWidth,windowHeight,function(v){f._createAndTriggerDownload(v.toDataURL("image/png"),
"page"+b+".png")}),f._pdfLoadingTask.start(),u.return();if(-1<h.indexOf(".js"))return pageResourceLoad(b,"normal",function(v){f._createAndTriggerDownload(v,"page"+b+".png")}),u.return();"downloadCurrent"===a&&isSupportWebp&&!bookConfig.isEncryptionBook&&(h=h.removeStartFrom("?"),-1<h.indexOf("webp")&&(h+="?x-oss-process=image/format,jpg"));if(h.match(/^none/))return n=document.createElement("canvas"),p=n.getContext("2d"),n.width=bookConfig.largePageWidth,n.height=bookConfig.largePageWidth,p.fillStyle=
bookConfig.pageBackgroundColor,p.fillRect(0,0,bookConfig.largePageWidth,bookConfig.largePageHeight),f._createAndTriggerDownload(n.toDataURL("image/png"),"page"+b+".png"),u.return();q="";"downloadCurrent"===a&&(q=b+".jpg");"downloadAttachments"===a&&(q=c||"");if(location.protocol.match("file:"))return/^\/\//i.test(h)&&(h="https://"+h),"English"==UserGroups&&"downloadAll"==a&&(t=window.location.href,h=h.replace(/bookId=\d+/,"bookUrl="+encodeURIComponent(t))),window.open(h),u.return();if("downloadAll"===
a)return"English"===UserGroups&&(r=window.location.href,h=h.replace(/bookId=\d+/,"bookUrl="+encodeURIComponent(r))),bookConfig.isEncryptionBook&&(x=BookRestrict.getRestrictToken(),h=appendParamToUrl(h,"code="+x)),window.open(h,"_blank"),u.return();if(isAndroid()&&isWeChat()||isPad())return window.open(h,"_blank"),u.return();if((!isPhone()&&!isPad()||_agent_.browser.safari)&&/fliphtml5|yunzhan365|anyflip|pubhtml5/.test(location.host))return f._createAndTriggerDownload(h,q),u.return();new DownloadFile({url:h,
...
f++)g.$jscomp$loop$prop$obj$8=d[f],k=void 0,g.$jscomp$loop$prop$obj$8.type&&"copy"==g.$jscomp$loop$prop$obj$8.type?(k=$("<a aria-label='"+g.$jscomp$loop$prop$obj$8.title+"' tabindex='0' class='share-item' id='copy_url' data-clipboard-action='copy' title='"+g.$jscomp$loop$prop$obj$8.title+"'>\n            <img alt='' src='"+g.$jscomp$loop$prop$obj$8.logo+"'/>\n          </a>"),copyHref(k[0]),BookEvent.trigger("customEvent",["copy_share_link"])):k=-1<g.$jscomp$loop$prop$obj$8.url.indexOf("javascript")?
...
3);2!=h.nextAddress&&(c=h.yieldResult);d=c;e={};for(g=0;g<a.itemArray.length;e={$jscomp$loop$prop$type$10:e.$jscomp$loop$prop$type$10},g++)f=a.itemArray[g],e.$jscomp$loop$prop$type$10=f.attr("data-type"),(k=d.find(function(l){return function(m){return m.type===l.$jscomp$loop$prop$type$10}}(e)))&&(-1<k.url.indexOf("javascript")?f.attr("onclick",k.url.remove("javascript:")+";return false;"):f.attr("href",k.url));h.jumpToEnd()})}.bind(this))},copyFrom:function(a){if(a){a.focus();a.select();try{document.execCommand("Copy")}catch(b){a=
