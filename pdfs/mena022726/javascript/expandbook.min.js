...
uiBaseURL+"note_edit.png),auto"});break;case AnnotationPannel.RECT:this.panelContainer.css({cursor:"url("+uiBaseURL+"note_edit.png),auto"});break;case AnnotationPannel.ROUND:this.panelContainer.css({cursor:"url("+uiBaseURL+"note_edit.png),auto"})}}.bind(this));this.penPaint(this.drawCanvas);this.penPaint(this.canvasPanel)},penPaint:function(a){var b=0,c=0;a.bind("pointerdown mousedown touchstart",function(g){(isPhone()||isPad())&&global.annotationPannel.bottomBox.css({display:"none"});global.colorPanel&&
global.colorPanel.visible&&global.colorPanel.curElem[0]==global.annotationPannel.showColor[0]&&global.colorPanel.hide();global.annotationCookie.currentSteps=[];2==global.annotationPannel.selectCase||3==global.annotationPannel.selectCase||4==global.annotationPannel.selectCase?(b=2,c=16):c=b=-4;if(isPhone()||isPad())c=b=0;g=this.getPosition(g,b,c);this.X=g.x;this.Y=g.y;if(5==global.annotationPannel.selectCase){var h=this.getNumberFromSlider("eraser")/2;isPhone()||isPad()||(h=0);this.X=g.x-h;this.Y=
g.y-h;var n={x:(g.x-h)/this.pageWidth,y:(g.y-h)/this.pageHeight,lineWidth:this.getNumberFromSlider("eraser")/this.pageWidth};this.showEraser(g.x-h,g.y-h,"block");this.eraserClear(g.x-h,g.y-h);global.annotationCookie.currentSteps.push(n)}this.noLeaveCvs=this.isDown=!0}.bind(this));a.bind("mouseup touchend pointerup",function(g){this.eraserCircle.css({display:"none"});this.noLeaveCvs&&(this.drawCaseEvent(g),this.isDown=this.noLeaveCvs=!1)}.bind(this));a.bind("mouseleave",function(g){this.noLeaveCvs=
...
100)};global.annotationCookie.setAllAnnoSteps(4,this.index-1,[g]);global.annotationCookie.stepIndex+=1;this.drawCtx.clearRect(0,0,this.pageWidth,this.pageHeight);this.drawEllipse(this.ctx,this.lastPos.x,this.lastPos.y);break;case 5:global.annotationCookie.setAllAnnoSteps(5,this.index-1,global.annotationCookie.currentSteps),global.annotationCookie.stepIndex+=1}this.lastPos=void 0}}.bind(this));a.bind("mousemove touchmove pointermove",function(g){if(this.isDown&&this.noLeaveCvs)switch(g=this.getPosition(g,
b,c),global.annotationPannel.selectCase){case AnnotationPannel.PEN:var h={startX:this.X/this.pageWidth,startY:this.Y/this.pageHeight,x:g.x/this.pageWidth,y:g.y/this.pageHeight,lineWidth:this.getNumberFromSlider("width")/this.pageWidth,opacity:this.getNumberFromSlider("alpha")/100,color:Color(global.annotationPannel.inputColor.val()).rgba(this.getNumberFromSlider("alpha")/100)};global.annotationCookie.currentSteps.push(h);this.drawLine(g.x,g.y);this.noLeaveCvs&&(this.lastPos=g);break;case AnnotationPannel.ERASER:var n=
this.getNumberFromSlider("eraser")/2;isPhone()||isPad()||(n=0);this.showEraser(g.x-n,g.y-n,"block");h={x:(g.x-n)/this.pageWidth,y:(g.y-n)/this.pageHeight,lineWidth:this.getNumberFromSlider("eraser")/this.pageWidth};global.annotationCookie.currentSteps.push(h);this.eraserClear(g.x-n,g.y-n);this.noLeaveCvs&&(this.lastPos=g);break;case AnnotationPannel.RECT:this.drawCtx.clearRect(0,0,this.pageWidth,this.pageHeight);this.drawRect(this.drawCtx,g.x,g.y);this.noLeaveCvs&&(this.lastPos=g);break;case AnnotationPannel.ROUND:this.drawCtx.clearRect(0,
0,this.pageWidth,this.pageHeight),this.drawEllipse(this.drawCtx,g.x,g.y),this.noLeaveCvs&&(this.lastPos=g)}}.bind(this))},getNumberFromSlider:function(a){switch(a){case "width":return isPhone()||isPad()?parseInt(global.annotationPannel.widthBox.find(".bubbleNum").text()):global.annotationPannel.inputWidth.val();case "eraser":return isPhone()||isPad()?parseInt(global.annotationPannel.eraserBox.find(".bubbleNum").text()):global.annotationPannel.inputEraserWidth.val();case "alpha":return isPhone()||
isPad()?parseInt(global.annotationPannel.alphaBox.find(".bubbleNum").text()):global.annotationPannel.inputAlpha.val()}},drawCaseEvent:function(a){switch(global.annotationPannel.selectCase){case 2:a=this.getPosition(a,2,16);global.annotationCookie.setAllAnnoSteps(2,this.index-1,global.annotationCookie.currentSteps);this.X=a.x;this.Y=a.y;global.annotationCookie.stepIndex+=1;break;case 3:a=this.getPosition(a,2,16);var b={startX:this.X/this.pageWidth,startY:this.Y/this.pageHeight,x:a.x/this.pageWidth,
y:a.y/this.pageHeight,lineWidth:this.getNumberFromSlider("width")/this.pageWidth,color:Color(global.annotationPannel.inputColor.val()).rgba(this.getNumberFromSlider("alpha")/100)};global.annotationCookie.setAllAnnoSteps(3,this.index-1,[b]);this.drawRect(this.ctx,a.x,a.y);this.drawCtx.clearRect(0,0,this.pageWidth,this.pageHeight);this.X=a.x;this.Y=a.y;global.annotationCookie.stepIndex+=1;break;case 4:a=this.getPosition(a,2,16);b={startX:this.X/this.pageWidth,startY:this.Y/this.pageHeight,x:a.x/this.pageWidth,
y:a.y/this.pageHeight,lineWidth:this.getNumberFromSlider("width")/this.pageWidth,color:Color(global.annotationPannel.inputColor.val()).rgba(this.getNumberFromSlider("alpha")/100)};global.annotationCookie.setAllAnnoSteps(4,this.index-1,[b]);this.drawEllipse(this.ctx,a.x,a.y);this.drawCtx.clearRect(0,0,this.pageWidth,this.pageHeight);this.X=a.x;this.Y=a.y;global.annotationCookie.stepIndex+=1;break;case 5:global.annotationCookie.setAllAnnoSteps(5,this.index-1,global.annotationCookie.currentSteps),global.annotationCookie.stepIndex+=
1}},getPosition:function(a,b,c,g){if(isPad()||isPhone())c=b=0,"note"==g&&(b=-this.pageWidth/25,c=-this.pageHeight/28);isMouseInput(a)||(c=b=0);var h;g=(null==(h=BookInfo.getBook().showPage.parameters)?void 0:h.toScale)||1;if(bookType===BookType.documentView_book||bookType===BookType.singleDocumentView_book)g=BookInfo.getBook().docViewBook.parameters.toScale||1;a=a.originalEvent&&a.originalEvent.changedTouches?a.originalEvent.changedTouches:a.changedTouches?a.changedTouches:[a];var n=document.documentElement.scrollLeft||
